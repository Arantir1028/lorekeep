...
from experiments.catalog import get_model_spec
...
def _lut_models(args: argparse.Namespace) -> str:
    ...
    return ",".join(get_model_spec(model).model_id for model in comma_list(args.models))
...
def _lut_command(args: argparse.Namespace, model: str) -> list[str]:
    lock = (
        str(args.lut_gpu_lock_path).strip()
        or str(Path(tempfile.gettempdir()) / f"waveslice_lut_rebuild_{args.gpu_profile}.lock")
    )
    options = (
        ("models", model),
