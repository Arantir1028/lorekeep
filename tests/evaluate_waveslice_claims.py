...
def _mk_requests(short_a_repeat: int, short_b_repeat: int, long_repeat: int) -> list[Req]:
    short_1 = (
        "Translate to French while preserving tone and meaning: "
        "I love machine learning systems and efficient serving pipelines. "
    ) * short_a_repeat
    short_2 = (
        "Continue the arithmetic pattern with explanation: "
        "10+20=30, 20+30=50, 30+40=70, 40+50=90. "
    ) * short_b_repeat
    long_1 = (
        "Summarize in one sentence: "
        + (
            "Artificial intelligence changes systems engineering and deployment, "
            "especially under heterogeneous LoRA workloads. "
        )
        * long_repeat
    )
    return [
...
def _measure_input_tokens(model_path: str, reqs: list[Req]) -> dict[str, int]:
    try:
...
def _fit_requests_to_context(
    *,
    ...
    for _ in range(8):
        ...
        tok_lens = _measure_input_tokens(model_path, reqs)
