...
def test_dynamic_chunk_clamped_to_bucket():
    slicer = WaveBaseSlicer(buckets=[32, 64, 128, 256, 512, 1024, 2048, 4096])
    c = slicer.choose_dynamic_chunk(
        short_len=100,
        long_len=3000,
...
def test_pack_short_and_long_chunk_meta():
    ...
    assert packed.shape == (192, 32)
    assert meta["long_offset"] == 256
    assert meta["rope_start"] == 256
    assert meta["rope_end"] == 384


def test_build_long_prefill_plan_tracks_offsets():
    slicer = WaveBaseSlicer(buckets=[32, 64, 128, 256, 512, 1024, 2048, 4096])
    ...
    assert [p.long_offset for p in plans] == [0, 1024]
    assert [p.chunk_len for p in plans] == [1024, 476]
    assert plans[0].rope_start == 0
    assert plans[1].rope_start == 1024


def test_chunk_token_ids_reports_rope_offsets():
    slicer = WaveBaseSlicer()
    chunk, meta = slicer.chunk_token_ids(list(range(32)), offset=10, chunk_len=7)
    assert list(chunk) == [10, 11, 12, 13, 14, 15, 16]
    assert meta == {
        "offset": 10,
        "chunk_len": 7,
        "remaining_len": 15,
