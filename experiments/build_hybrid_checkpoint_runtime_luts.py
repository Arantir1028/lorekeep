...
def _portable_snapshot_path(model_id: str, snapshot: str | Path) -> str:
    """Return a snapshot path relative to the Hugging Face hub cache."""
    value = str(snapshot).strip()
    if not value:
        return value
    ...
    if value.startswith("hf://") and "@" in value:
        ...
        revision = Path(value).name
    if not model_id:
        return revision
    cache_name = "models--" + model_id.replace("/", "--")
    return f"{cache_name}/snapshots/{revision}"


def _portable_registry_row(row: dict[str, Any]) -> dict[str, Any]:
    portable = dict(row)
    model_id = str(portable.get("model_id") or "")
    revision = portable.pop("revision", None)
    snapshot = portable.get("snapshot") or revision
    if snapshot:
        portable["snapshot"] = _portable_snapshot_path(model_id, snapshot)
    for field in _REGISTRY_ARTIFACT_FIELDS:
        if portable.get(field):
            portable[field] = Path(str(portable[field])).name
    return portable
...
def _selected_models(local: list[tuple[str, Path]], models_arg: str) -> list[tuple[str, Path]]:
    ...
    selected = [
        ...
        if model in chosen or cfg.lut_name_from_model_ref(model) in chosen
    ]
    known = {
        value
        for model, _ in selected
        for value in (model, cfg.lut_name_from_model_ref(model))
    }
...
def _runtime_calibration(
    *,
    ...
    output = Path(cfg.DATA_DIR) / f"runtime_calibration_{cfg.lut_name_from_model_ref(model_id)}.json"
...
def _write_registry(path: Path, rows: list[dict[str, Any]]) -> None:
    ...
    merged = {
        str(row["model_id"]): _portable_registry_row(row)
        for row in existing
    ...
    with gpu_experiment_lock(
        ...
        for model_id, snapshot in targets:
            config, lut_name = (
                ...
                cfg.lut_name_from_model_ref(model_id),
            )
            ...
            row = dict(
                model_id=model_id,
                lut_name=lut_name,
                snapshot=_portable_snapshot_path(model_id, snapshot),
                aliases=aliases,
                **params,
            ...
            try:
                ...
                runtime["model_ref"] = model_id
                runtime["snapshot"] = _portable_snapshot_path(model_id, snapshot)
                ...
                row.update(
                    status="ok",
                    runtime_meta=runtime_path.name,
                    runtime_sanity=sanity_path.name,
                    base_raw_profile=Path(backups["raw"]).name,
                    base_gain=Path(backups["gain"]).name,
                    base_penalty=Path(backups["penalty"]).name,
                    hybrid_raw_profile=Path(paths["raw"]).name,
                    hybrid_gain=Path(paths["gain"]).name,
                    hybrid_penalty=Path(paths["penalty"]).name,
                )
            except Exception as error:
                row.update(status="failed", error=repr(error))
                if _runtime_sanity_path(lut_name).exists():
                    row["runtime_sanity"] = _runtime_sanity_path(lut_name).name
