...
class ModelProfiler:
    def __init__(self, model_name: str, device: torch.device, dtype: torch.dtype):
        ...
        self.pool_Q = torch.randn(
            cfg.BATCH_SIZE, self.q_heads, max_b, self.head_dim, device=self.device, dtype=self.dtype
        )
        self.pool_K = torch.randn(
            cfg.BATCH_SIZE,
            self.kv_heads,
            max_b,
            self.head_dim,
        ...
        self.pool_X = torch.randn(
            cfg.BATCH_SIZE, max_b, self.d_model, device=self.device, dtype=self.dtype
        )
        self.pool_W_A = torch.randn(
            cfg.BATCH_SIZE, self.d_model, cfg.LORA_RANK, device=self.device, dtype=self.dtype
        )
        self.pool_W_B = torch.randn(
            cfg.BATCH_SIZE, cfg.LORA_RANK, self.d_model, device=self.device, dtype=self.dtype
        )
        self.pool_Out = torch.empty(
            cfg.BATCH_SIZE, max_b, self.d_model, device=self.device, dtype=self.dtype
        )
        ...
        with sdpa_kernel(backends=[SDPBackend.FLASH_ATTENTION]):
            for _ in range(WARMUP_ITERS):
                for i in range(k - 1):
                    simulate_attention(
                        q_chunks[i],
                        K_full[:, :, : history_lens[i], :],
                        V_full[:, :, : history_lens[i], :],
                        self.q_heads,
                        self.kv_heads,
                    )
            torch.cuda.synchronize()
    ...
    def run(self):
        print(f"\n{'=' * 70}")
        print(
            f"=== Profiling {self.model_name} ({self.params['attn_type']}) on {torch.cuda.get_device_name(self.device)} ==="
        )
        ...
        for s_l in tqdm(cfg.BUCKETS, desc="T_read_amp"):
            ...
            for s_c in cfg.BUCKETS:
                profile_data["T_read_amp"][s_l][s_c] = (
                    self.measure_read_amp_sum(s_l, s_c) if s_c < s_l else 0.0
                )
